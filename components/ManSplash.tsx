...
// A distinctive but FAST brand reveal: user name (when authenticated) -> logo ->
// subtle intelligent animation -> wordmark -> tagline -> transition into interface.
...
export default function ManSplash({
  ...
  const [stage, setStage] = useState<0 | 1 | 2>(0); // 0 name, 1 MAN, 2 tagline
  ...
  const displayName = name && name.length ? name.replace(/_/g, " ").trim() : "MAN";
  ...
  return (
    <div className="man-splash" role="status" aria-label="Loading MAN"
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 18, transform: "translateY(-4%)" }}>
        {stage === 0 && (
          <div className="splash-rayhan">
            <span className="splash-rayhan-name">{displayName}</span>
      ...
      <style>{`
        ...
        .splash-rayhan-name {
          font-size: clamp(40px, 12vw, 92px); font-weight: 800; letter-spacing: .01em;
          background: var(--man-grad); -webkit-background-clip: text; background-clip: text;
          color: transparent; line-height: 1; text-align: center; max-width: 92vw; overflow-wrap: anywhere;
        }
